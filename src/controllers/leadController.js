...
exports.submitLead = async (req, res) => {
  ...
  if (!email || !projectTitle || !description || !budgetRange) {
    return res.status(400).json({
      message:
        "Name, email, phone, project category, project title, description, and budget range are required.",
    });
  }
  ...
  try {
    ...
    if (existingClient) {
      if (existingClient.isEmailVerified && existingClient.isActive) {
        ...
        return res.status(200).json({
          message:
            "An unverified account with this email exists. We've resent the account setup link. Please check your email to set your password and activate your account.",
        });
      }
    } else {
      const verificationToken = crypto.randomBytes(32).toString("hex");
    ...
    res.status(201).json({
      message:
        "Project quote submitted successfully! Please check your email to complete your account setup and activate your DIGIHUB AUST account.",
      lead: {
        id: newLead.id,
        email: newLead.email,
    ...
    if (error.code === "P2002" && error.meta?.target?.includes("email")) {
      return res.status(409).json({
        message:
          "An account with this email already exists. If you already have an account, please login. Otherwise, check your email for the account setup link.",
      });
    }
...
exports.assignPartnerToLead = async (req, res) => {
  try {
    ...
    if (
      ...
      ].includes(existingLead.status)
    ) {
      return res.status(400).json({
        success: false,
        message: `Cannot assign partner to lead with status: ${existingLead.status}.`,
      });
    }
    ...
    res.status(200).json({
      success: true,
      message: "Partner assigned to lead successfully.",
      data: updatedLead,
    });
  } catch (error) {
...
exports.deleteLeadByAdmin = async (req, res) => {
  ...
  try {
    ...
    if (lead && lead.projectId) {
      return res.status(409).json({
        message:
          "Cannot delete lead. It has already been converted to a project. Consider archiving instead.",
      });
    }
    ...
    if (error.code === "P2003") {
      return res.status(409).json({
        message:
          "Cannot delete lead due to existing related data (e.g., foreign key constraint). Consider soft deleting or manually removing relationships.",
      });
    }
    res.status(500).json({ message: "Internal server error" });
  }
...
exports.resentAdminOfferPartner = async (req, res) => {
  const { id } = req.params;
  const { partnerProposedCost, partnerId, timeline, notes } = req.body; // Input from Admin
...
exports.sendOfferToClient = async (req, res) => {
  ...
  try {
    ...
    if (!timeline) {
      return res
        .status(400)
        .json({ success: false, message: "Timeline is required!" });
    }
    const lead = await prisma.lead.findUnique({
      where: { id: leadId },
      select: {
    ...
    if (
      ...
      return res.status(400).json({
        success: false,
        message: `Cannot send offer for lead with status: ${lead.status}. Lead must have a partner offer proposed or be assigned.`,
      });
    }
    ...
    if (lead.partnerProposedCost === null) {
      return res.status(400).json({
        success: false,
        message:
          "Partner's proposed cost is missing. Cannot finalize client offer.",
      });
    }
...
exports.clientAcceptOffer = async (req, res) => {
  try {
    ...
    if (lead.status !== "OFFER_SENT_TO_CLIENT") {
      return res.status(400).json({
        success: false,
        message: `Offer cannot be accepted. Current lead status is "${lead.status}".`,
      });
    }
    ...
    if (
      ...
      return res.status(400).json({
        success: false,
        message:
          "Missing offer details or required lead information for project creation. Please contact support.",
      });
    }
